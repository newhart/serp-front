<template>
  <div>
    <div v-if="FileAdd === ''">
      <div class="form-input-send position-relative d-flex align-center">
        <v-text-field
          label="Une bonne nouvelle?"
          outlined
          clearable
          solo
          class="input-globale avecBorger"
          height="45px"
          min-height="45px"
          hide-details
    ...
    <div class="addphotos" v-else>
      ...
      <div class="d-flex justify-end">
        ...
        <v-btn rounded color="primary" class="font12" @click="post()">
          Publier
        </v-btn>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    post() {
      ...
      this.images = [];
      this.content = "";
      this.$emit("status", "importation");
      this.$axios
        .post("api/actualites/create", {
          content: var_content,
          images: var_img,
          id_group: this.id_group,
          background_id: this.backgroundActive
            ? this.backgroundActive.id
            : null,
        })
        .then((res) => {
          if (res.data.success) {
            this.$emit("status", "success");
            this.$emit("newPub", res.data.newPub);
            this.$emit("newPubGroup", true);
            var_content = "";
            var_img = [];
          }
        });
